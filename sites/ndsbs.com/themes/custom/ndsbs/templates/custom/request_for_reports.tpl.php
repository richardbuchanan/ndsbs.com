  ...
  foreach ($cart_data as $carddata) {
    ...
    if (!in_array($carddata->tid, $main_texonomy_term)) {
      ...
      <tr>
        ...
        ?>
        <td class="bold">$<span id="sub_current_amount<?php print $carddata->cid; ?>"><?php print number_format(($subreport_amount + $carddata->notary_amount), 2); ?></span>
        </td>
        <td>
...
<div>
  ...
  <div class="uk-alert-primary" uk-alert><?php print $message; ?></div>

  <p>If you need a letter or report by regular mail choose from options below, purchase and provide the mailing address</p>

  <form>
    <fieldset class="uk-fieldset">
      <legend class="uk-legend">Shipping Method</legend>
      <div class="uk-margin">
        <div class="uk-form-controls">
          <label><input type="radio" name="shipping_method" class="uk-radio" checked> Express Mail</label>
          <br>
          <label><input type="radio" name="shipping_method" class="uk-radio"> Regular U.S. Mail (Free)</label>
        </div>
      </div>
    </fieldset>
  </form>
  ...
  <div class="request2">
    <?php print get_user_shipping_address_form(); ?>
