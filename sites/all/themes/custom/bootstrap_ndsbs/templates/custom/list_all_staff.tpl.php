  ...
  foreach ($clients_id as $rec) {
    ...
    <tr>
      <td>
        <a href="/user/<?php print $user_info->uid; ?>/edit">
          <?php if ($user_info->field_profile_picture['und'][0]['uri'] <> '') { ?>
            <img
              src="<?php print image_style_url('thumbnail', $user_info->field_profile_picture['und'][0]['uri']) ?>">
          <?php }
          else { ?>
            <img
              src="<?php print image_style_url('thumbnail', 'public://default_images/UserDefault.jpg') ?>">
          <?php } ?>
        </a>
      </td>
      <td>
        <b>First
          Name-</b> <?php print l(t($user_info->field_first_name['und'][0]['value']), 'user/' . $user_info->uid . '/edit'); ?>
        <br/>
        <b>Middle
          Name-</b> <?php print $user_info->field_middle_name['und'][0]['value']; ?>
        <br/>
        <b>Last
          Name-</b> <?php print $user_info->field_last_name['und'][0]['value']; ?>
      </td>
      <td>
        <b>Gender-</b> <?php print $user_info->field_gender['und'][0]['value']; ?>
        <br/>
        <b>Date of
          Birth-</b> <?php print $user_info->field_month['und'][0]['value']; ?>
        -<?php print $user_info->field_dobdate['und'][0]['value']; ?>
        -<?php print $user_info->field_year['und'][0]['value']; ?>
        <br/>
        <b>Status-</b> <?php if ($user_info->status == 1) {
          print 'Active';
        }
        else {
          print 'Blocked';
      ...
      <td>
        <?php
        $options = array(
          'query' => array('destination' => 'user/staff/list'),
